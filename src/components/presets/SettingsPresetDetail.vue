<template>
    <div v-if="hasPresetData" class="preset-detail-section mt-3 pt-3 border-t border-gray-200">
        <!-- Current preset indicator -->
        <div v-if="currentPreset" class="flex items-center justify-between mb-2">
            <div class="flex items-center text-xs text-green-600">
                <Icon name="check" size="xs" class="mr-1" />
                <span class="font-medium">Set by {{ currentPreset.title }}</span>
            </div>
            <button
                @click="expanded = !expanded"
                class="text-xs text-gray-500 hover:text-gray-700 focus:outline-none"
            >
                {{ expanded ? 'Hide' : 'Show' }} preset options
            </button>
        </div>

        <!-- Toggle button when no current preset -->
        <div v-else-if="presetValues.length > 0" class="flex items-center justify-between mb-2">
            <span class="text-xs text-gray-600">{{ presetValues.length }} preset value{{ presetValues.length > 1 ? 's' : '' }} available</span>
            <button
                @click="expanded = !expanded"
        ...
        <div v-if="expanded" class="space-y-2">
            ...
            <div v-if="presetValues.length > 0" class="space-y-1">
                <p class="text-xs font-medium text-gray-700 mb-1">Preset Values:</p>
                <div class="space-y-1">
                    <div
                    ...
                    >
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                                <span class="font-mono text-gray-900">{{ formatValue(presetValue.value) }}</span>
                                <span class="text-gray-500">
                                    ({{ presetValue.count }} preset{{ presetValue.count > 1 ? 's' : '' }})
                                </span>
                            </div>
                            <div class="flex flex-wrap gap-1 mt-1">
                                <span
                                    v-for="presetId in presetValue.presets.slice(0, 3)"
                                    :key="presetId"
                                    class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-gray-200 text-gray-700"
                                >
                                    {{ getPresetTitle(presetId) }}
                                </span>
                                <span v-if="presetValue.presets.length > 3" class="text-xs text-gray-500">
                        ...
                        <button
                            ...
                            @click="$emit('apply-preset-value', presetValue.value)"
                        ...
                        <span v-else class="ml-2 px-2 py-1 text-xs font-medium text-green-600 bg-green-50 border border-green-200 rounded">
                            Current
                        </span>
                    </div>
                </div>
            ...
            <div v-if="settingOrigins.length > 0" class="pt-2 border-t border-gray-100">
                <p class="text-xs font-medium text-gray-700 mb-1">Applied by:</p>
                <div class="space-y-1">
                    <div
                    ...
                    >
                        <div class="flex items-center gap-2">
                            <span class="font-medium text-gray-900">{{ getPresetTitle(origin.presetId) }}</span>
                            <span class="text-gray-500">{{ formatDate(origin.appliedAt) }}</span>
                        </div>
                        <button
            ...
            <!-- Quick actions -->
            <div v-if="presetValues.length > 0" class="pt-2 border-t border-gray-100">
                <div class="flex items-center gap-2">
                    <button
                        v-if="mostCommonValue && !isCurrentValue(mostCommonValue.value)"
                        @click="$emit('apply-preset-value', mostCommonValue.value)"
                        class="text-xs font-medium text-accent-600 hover:text-accent-700 focus:outline-none"
                    >
                        Use most common ({{ formatValue(mostCommonValue.value) }})
                    </button>
                    <button
...
const emit = defineEmits(['apply-preset-value', 'revert-preset', 'reset-setting']);
...
const currentPreset = computed(() => {
    ...
    const sortedOrigins = [...props.settingOrigins].sort((a, b) => 
